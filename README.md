...
A DIY touchscreen macro deck ("CydDeck") built from a $10 **ESP32-2432S028R "Cheap Yellow Display" (CYD)** — a 2.8" 240×320 touchscreen ESP32 board. The screen shows a now-playing header (player status + track info) above 10 icon buttons (2 columns × 5 rows, portrait); tapping a button sends the button ID over USB serial to a small Python bridge on the PC, which runs the mapped action: media controls, launching apps, or any shell command.

**Everything about the buttons — label, icon, color, and action — lives in one JSON file on the PC.** The bridge pushes the layout to the device over serial, so you never reflash to reconfigure. Edit `cyddeck_config.json`, save, and the deck redraws within a second.
...
│ ⏮ Prev  │ 🔊 Vol + │           │
│ 🔉 Vol - │ 🔇 Mute  │      cyddeck_bridge.py ──▶ action
│ ⌨ Term  │ </> Code │           ▲
...
```
cyddeck/cyddeck.ino         Firmware: buttons, icons, touch, serial protocol
...
cyddeck_bridge.py               PC bridge: serial listener + config pusher
cyddeck_config.json             All button definitions and actions
...
```bash
arduino-cli compile --fqbn esp32:esp32:esp32 cyddeck
arduino-cli upload  --fqbn esp32:esp32:esp32 -p /dev/ttyUSB0 cyddeck
```
...
.venv/bin/python cyddeck_bridge.py
```
...
.venv\Scripts\python cyddeck_bridge.py
```

You should see `Pushed layout for 10 buttons` and the deck redraw with your configured layout. Tap away.

### 3b. Windows without Python (bridge exe)

For a no-toolchain setup on Windows, skip the venv entirely:

1. Install the [CH340 driver](https://www.wch-ic.com/downloads/CH341SER_ZIP.html) if the board doesn't show up as a COM port.
2. Download `cyddeck-bridge.exe` from the [Releases page](../../releases) and put it in any folder.
3. Double-click it. A console window shows status, and a `cyddeck_config.json` appears next to the exe — edit that file to customize buttons; changes apply live.

To build the exe yourself (the `build-windows-exe` GitHub Action does the same on each release):

```powershell
pip install -r requirements.txt pyinstaller
pyinstaller --onefile --name cyddeck-bridge cyddeck_bridge.py
# result: dist\cyddeck-bridge.exe
```

## Configuring buttons

Everything is in `cyddeck_config.json`. Each button (IDs `0`–`9`, left-to-right then top-to-bottom) looks like:
...
Top-level keys: `port` and `baud` (leave at `115200`). `"port": "auto"` picks the first USB-serial adapter it finds; if you have several, set it explicitly — `"COM3"` on Windows, `"/dev/ttyUSB0"` on Linux. If the config file doesn't exist, the bridge creates a starter one (with OS-appropriate commands) next to itself on first run.
...
Icons are drawn with TFT primitives in the firmware's `drawIcon()`. Adding a new one is ~10 lines of C++ in `cyddeck.ino` (draw inside a 22×22 box centered on `cx,cy`, add a name to `ICON_NAMES`), and then it's usable from the JSON like any other.
...
|---|---|---|
| device → PC | `CYDDECK READY 10` | Booted; layout is at firmware defaults, push config |
...
| PC → device | `PING` | Ask the device to re-announce `CYDDECK READY` |
...
3. Targets are at screen (30,30), (210,30), (30,290). Fit the raw values linearly and update the `TOUCH_RX_*` / `TOUCH_RY_*` defines in `cyddeck.ino` (comments there explain which raw axis maps to which screen axis), then reflash the deck.

## Run at login (systemd user service)

```bash
mkdir -p ~/.config/systemd/user
cat > ~/.config/systemd/user/cyddeck.service <<'EOF'
[Unit]
Description=CYD deck serial bridge

[Service]
WorkingDirectory=%h/cyddeck
ExecStart=%h/cyddeck/.venv/bin/python cyddeck_bridge.py
Restart=on-failure

[Install]
WantedBy=default.target
EOF
systemctl --user enable --now cyddeck
```

Adjust `WorkingDirectory`/`ExecStart` if the repo lives elsewhere. Logs: `journalctl --user -u cyddeck -f`.
...
- **Deck shows default labels** — the firmware booted without the bridge running; start the bridge and it re-pushes on the `CYDDECK READY` banner.
...
- Firmware defaults (the layout shown before the bridge connects) are in the `buttons[]` array in `cyddeck.ino`; they're cosmetic — the JSON always wins once the bridge connects.

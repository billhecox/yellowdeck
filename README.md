...
│ ⏮ Prev  │ 🔊 Vol + │           │
│ 🔉 Vol - │ 🔇 Mute  │      yellowdeck_bridge.py ──▶ action
│ ⌨ Term  │ </> Code │           ▲
...
```
yellowdeck/yellowdeck.ino         Firmware: buttons, icons, touch, serial protocol
...
```bash
arduino-cli compile --fqbn esp32:esp32:esp32 yellowdeck
arduino-cli upload  --fqbn esp32:esp32:esp32 -p /dev/ttyUSB0 yellowdeck
```
...
.venv/bin/python yellowdeck_bridge.py
```
...
.venv\Scripts\python yellowdeck_bridge.py
```
...
2. Download `yellowdeck-bridge.exe` from the [Releases page](../../releases) and put it in any folder.
3. Double-click it. A console window shows status, and a `yellowdeck_config.json` appears next to the exe — edit that file to customize buttons; changes apply live.
...
Everything is in `yellowdeck_config.json`. Each button (IDs `0`–`9`, left-to-right then top-to-bottom) looks like:
...
Icons are drawn with TFT primitives in the firmware's `drawIcon()`. Adding a new one is ~10 lines of C++ in `yellowdeck.ino` (draw inside a 22×22 box centered on `cx,cy`, add a name to `ICON_NAMES`), and then it's usable from the JSON like any other.
...
|---|---|---|
| device → PC | `YELLOWDECK READY 10` | Booted; layout is at firmware defaults, push config |
...
| PC → device | `PING` | Ask the device to re-announce `YELLOWDECK READY` |
...
3. Targets are at screen (30,30), (210,30), (30,290). Fit the raw values linearly and update the `TOUCH_RX_*` / `TOUCH_RY_*` defines in `yellowdeck.ino` (comments there explain which raw axis maps to which screen axis), then reflash the deck.

## Run at login (systemd user service)

```bash
mkdir -p ~/.config/systemd/user
cat > ~/.config/systemd/user/yellowdeck.service <<'EOF'
[Unit]
Description=CYD deck serial bridge

[Service]
WorkingDirectory=%h/yellowdeck
ExecStart=%h/yellowdeck/.venv/bin/python yellowdeck_bridge.py
Restart=on-failure

[Install]
WantedBy=default.target
EOF
systemctl --user enable --now yellowdeck
```

Adjust `WorkingDirectory`/`ExecStart` if the repo lives elsewhere. Logs: `journalctl --user -u yellowdeck -f`.
...
- **Deck shows default labels** — the firmware booted without the bridge running; start the bridge and it re-pushes on the `YELLOWDECK READY` banner.
